...
def leapfrog_(q, p, dVdq, potential, path_len, step_size):
    """Leapfrog integrator for Hamiltonian Monte Carlo.
    Parameters
    ----------
    q : np.floatX
    ...
    p -= step_size * dVdq / 2  # half step
    for _ in torch.arange(path_len):#np.arange(np.round(path_len / step_size) - 1):
        q += step_size * p  # whole step
        V, dVdq = potential(q)
        p -= step_size * dVdq  # whole step
    q += step_size * p  # whole step
...
def hamiltonian_monte_carlo_da_(
    numiter,
    burnin,
    thinning,
    potential,
    ...
    step_size =torch.tensor(initial_step_size).to(device)
    step_size_tuning = DualAveragingStepSize_(step_size)

    with trange(numiter) as tr:
        for t in tr:
            p0=momentum.sample(initial_position.shape).to(device)
            ...
            q_new, p_new, final_V, final_dVdq = integrator(
                ...
                initial_potential_grad,
                potential,
                path_len= 2* torch.rand(())*path_len,  #We jitter the path length a bit
                step_size=step_size,
            )
            ...
            # Set accept prob to 0.0 if energy_change is `NaN` which may be
            # the case for a diverging trajectory when using a large step size.
            if torch.isnan(q_new).sum() or torch.isnan(p_new).sum():
                p_accept = 0.           
            else:
                start_log_p = momentum.log_prob(p0).sum() - initial_potential
                new_log_p = momentum.log_prob(p_new).sum() - final_V
                energy_change = new_log_p - start_log_p
                
                if torch.isnan(energy_change.exp()) or energy_change > max_energy_change :  
                    p_accept = 0.  
                else:
                    p_accept = min(1, energy_change.exp())
            ...
            averaged_step=step_size_tuning.log_averaged_step.exp()
            if t % 100 ==0:
                scores['averaged_steps'].append(averaged_step)
                scores['p_accept'].append(p_accept)
                scores['step_size'].append(step_size)
            ...
            tr.set_postfix(pot=initial_potential.item(), step_av=averaged_step.item(), step=step_size.item(), norm=q_new.norm().item())
...
class DualAveragingStepSize_:
    def __init__(
        ...
        kappa: float in (0.5, 1]
            The smaller kappa is, the faster we forget earlier step size iterates
        """
        self.mu = (10 * initial_step_size).log()
        self.target_accept = target_accept
        self.gamma = gamma
        self.t = torch.as_tensor(t0)
        self.kappa = kappa
        self.error_sum = 0
        self.log_averaged_step = torch.tensor(0.)
    ...
    def update(self, p_accept):
        ...
        """
        self.error_sum += self.target_accept - p_accept
        log_step = self.mu - self.error_sum / (self.t.sqrt() * self.gamma)
        eta = self.t ** -self.kappa
        self.log_averaged_step = (
            eta * log_step + (1 - eta) * self.log_averaged_step
        )
        self.t += 1
        return log_step.exp(), self.log_averaged_step.exp()
