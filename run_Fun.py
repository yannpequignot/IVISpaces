...
if __name__ == "__main__":
    ...
    with mlflow.start_run():
        ...
        for dataset in datasets:
            print(dataset)
            models=run(dataset, n_samples_FU=n_samples_FU, ratio_ood=args.ratio_ood, sigma_input=args.sigma_input) 
            print(dataset+': done :-)')
            FuNmodels.update({dataset:models})
            torch.save(FuNmodels, 'Results/FuNmodels.pt')

        mlflow.log_artifact('Results/FuNmodels.pt')
        ...
        for d in datasets:
                metrics=run_metrics(d, 'FuNNeVI') 
                print(d+': done :-)')
                results.update(metrics)
                torch.save(results, 'Results/FuNmetrics.pt')

        mlflow.log_artifact('Results/FuNmetrics.pt')
