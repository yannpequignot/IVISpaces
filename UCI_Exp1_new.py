...
if __name__ == "__main__":
    ...
    if args.set == "large":
    # large ##
        file_name = 'Results/Paper/Exp1/Exp1_large' + date_string+args.dataset
 ...
 #   MODELS = {dataset: [] for dataset in datasets}
    ...
    if dataset== 'navalC':
         MODELS=torch.load(path+"Exp1_large2021-02-04-08:34navalC_models.pt",map_location=device)
#     if dataset== 'concrete':
...
#         MODELS[dataset][i][1].update({"FuNN-MFVI": [model.state_dict(), logs, time]})
